...
public class EditPlanDialogue extends JFrame {
	...
	{
		new EditPlanDialogue(caller, plan);
	}
	
	private EditPlanDialogue(GUI caller, BeanPlan plan)
	{
	...
	private void createGUI()
	{
		...
		datePartIn.setSelectedIndex(plan.getDate().PART().ordinal());
		...
		dateMonthIn.setSelectedIndex(plan.getDate().MONTH().ordinal()-1);
		dateSubPanel.add(dateMonthIn);
		JSpinner dateYearIn = new JSpinner(new SpinnerNumberModel(plan.getDate().YEAR(), 2000, 9999, 1));
		...
		receiveDatePartIn.setSelectedIndex(plan.getReceiveDate().PART().ordinal());
		...
		receiveDateMonthIn.setSelectedIndex(plan.getReceiveDate().MONTH().ordinal()-1);
		receiveDateSubPanel.add(receiveDateMonthIn);
		JSpinner receiveDateYearIn = new JSpinner(new SpinnerNumberModel(plan.getReceiveDate().YEAR(), 2000, 9999, 1));
		...
		for(BeanPlatform p : Platforms.getInstance().PLATFORMS())
		{
			if(p.NAME().equals(plan.getPlatform().NAME()))
			{
				platformIn.setSelectedItem(p);
				break;
			}
		}
		...
		platformEdit.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){			
				...
				int option = JOptionPane.showConfirmDialog(EditPlanDialogue.this, inputs, "Plattform hinzufügen", JOptionPane.OK_CANCEL_OPTION);
		...
		typeIn.setSelectedIndex(plan.getType().ordinal());
		...
		stateIn.setSelectedIndex(plan.getState().ordinal());
		...
		JButton add = new JButton("Speichern");
		bottom.add(add);
		add.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				...
				{
					Manager.getInstance().deletePlan(plan);
